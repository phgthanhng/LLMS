...
public class LibrarianDAO {
	...
	/**
	 * create a new entry in the Books table to add a new book to the catalog set
	 * "Issued" attribute to zero and addedDate to the current date
	 * 
	 * @param book
	 */
	public void addBook(BooksModel book) {
		...
		try {
			...
			String sql = "INSERT INTO Books (SN, Title, Author, Publisher, Quantity, AddedDate) VALUES (?, ?, ?, ?, ?, DATE())";
			...
			if (rowAffected == 1) {
				System.out.println(messages.getString("addSuccess"));
			} else {
				System.out.println(messages.getString("addFailed"));
			}

		} catch (SQLException e) {
			System.out.println(messages.getString("addBookErr") + " [" + e.getMessage() + "]");
			try {
				con.rollback();
	 ...
	 */
	public boolean issueBook(BooksModel book, StudentModel student) {
		currentLocale = mainView.getCurrentLocale();
		messages = mainView.getMessages();
		...
		try {
			...
			newStm.close();
			con.commit();
			System.out.println(messages.getString("issueSuccess"));
			return true;
		} catch (SQLException e) {
	 ...
	 * @return true if success
	 */
	public boolean toReturnBook(BooksModel book, StudentModel student) {
		currentLocale = mainView.getCurrentLocale();
		messages = mainView.getMessages();
		Scanner console = new Scanner(System.in);
	...
	public static List<Object> viewIssuedBooks() {
		...
		try {
			...
			while (rs.next()) {
				...
				+ " = " + rs.getString("Title") + ", " + messages.getString("author") + " = " + rs.getString("Author") 
				+ ", "+ messages.getString("stdId") + " = " + rs.getString("StudentId") + ", "+ messages.getString("issuedDate") + " = " + rs.getString("IssuedDate");
